...
public sealed partial class MusicGen
{
	...
	void RenderDrumBar( int barTick, int barTicks, int fillTick, Rng noise )
	{
		...
		if ( barTick == _sectionTick && noise.Chance( _groove.CrashOnOne ) )
			RenderCymbal( _time.TickToSample( barTick ),
				_c.CrashVol * KitGain( barTick, 1f, 0.5f ), CrashBright );
		...
		for ( int i = 0; i < play.Count; i++ )
		{
			...
			if ( _crashRide )
				...
				RenderCymbal( at, _c.CrashVol * kit * (cell == DrumGroove.Open ? 0.55f : 0.38f),
					CrashDark );
			else if ( _ride )
				...
				RenderCymbal( at, amp, cell == DrumGroove.Open ? RideBell : RideBow( i ) );
			else
			...
			if ( cell == 0 && !sparse && noise.Chance( busy ) )
			{
				...
				if ( _crashRide ) RenderCymbal( sixAt, _c.CrashVol * kit * 0.22f, CrashDark );
				else if ( _ride ) RenderCymbal( sixAt, amp * 0.4f, RideBow( i + 1 ) );
	...
	void RenderFill( int fromTick, int toTick, Rng noise, Rng rng )
	{
		...
		for ( int b = 0; b < beats; b++ )
		{
			...
			for ( int i = 0; i < FillCells; i++ )
			{
				...
				if ( d < tomShare )
					...
					RenderCymbal( t, _c.HatVol * gain, RideBow( i ) );
		...
		RenderCymbal( _time.TickToSample( toTick ), _c.CrashVol * KitGain( toTick, 1f, 0.35f ),
			rng.Chance( 0.4f ) ? CrashDark : CrashBright );
	}
}
